...
- Ordinary appointment conflicts stop before wave creation and use read-only lookups. **Repair appointment** is a separate, confirmed production action for one outbound AZ02 load. It checks allocation, ownership, the saved shipment and empty picks first. A correctly linked unallocated wave is reused. A missing wave is created with the exact saved name. A different unallocated wave is deleted only after verifying that it belongs exclusively to this load and shipment; its removal must be verified before creating the intended wave. Once the intended wave and links are verified, the existing appointment is updated to the saved date, time, door group and carrier with shipping equipment type SHIP. Its ID and load association are retained. The workbook is not rerun.
- Allocated, active or shared waves, recurring or checked-in appointments, missing allocation evidence, and changed assignments stop for review. Wave repair requires exactly one saved shipment. An existing saved wave name with an inconsistent load link stops for manual review rather than creating a duplicate. The repair uses an exclusive server-side lock and saves its audit before every request. Blue Yonder reads and writes are separate requests, so another user's concurrent changes can still cause a protected stop or an uncertain result. A failed or uncertain write is not retried automatically. Review every repaired load manually, including successful repairs.
- Run results retain their original production totals. Repair history records the saved wave and shipment, old wave deletion, wave creation or reuse, appointment movement, verified time, door group, carrier, shipping type and links. A failed wave setup stops before the appointment update. If an appointment update fails afterward, confirmed wave changes remain recorded as partial completion and the run requires manual review. The website and repair email show these actions separately from the original run outcome.
- **Inbounds to link** lists receiving appointments created for missing 311, 660 or ITRN/ITRA references. Only a structured 404 lookup permits this fallback; authentication and server errors stop for review. The appointment uses the saved time, door group, carrier and RCV type. The full original reference goes on the first line of **Appointment Notes**, followed by “not in system when appointment was made” on the second line. Notes are saved separately through the appointment-notes resource, as the Blue Yonder form does. A deterministic 10-character external appointment reference checks for a matching earlier bot appointment before creation; it never replaces the full reference in the notes. Matching existing appointments must pass a fresh details and notes check. Changed, active or ambiguous matches stop. Existing notes are never overwritten, and both appointment details and saved notes must be verified before reporting success. Database failures are not resubmitted as overridable warnings. The nonexistent truck is not assigned; an operator links it manually when available. The website, CSV export and email retain this follow-up list. Preflight checks and writes are separate API requests; do not launch overlapping production runs or retry an uncertain submission.
