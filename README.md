...
- Ordinary appointment conflicts stop before wave creation and use read-only lookups. **Repair appointment** is a separate, confirmed production action for one outbound AZ02 load. It uses the original result's saved wave name and shipment ID. A correctly linked unallocated wave is reused. If no wave is linked, it checks that the saved name is unused, the shipment belongs to the load, and no wave or pick work exists before creating the wave. An old unallocated wave with a different name is deleted and replaced with the saved wave. Creation must finish and the wave, load and shipment links must be verified before moving the existing appointment to the saved date, time and slot. The appointment ID is retained; an already correct appointment needs no update. The workbook is not rerun.
...
- Run results retain their original production totals. Repair history records the saved wave and shipment, original appointment time, wave creation or reuse, verified links, any old-wave deletion, and the verified appointment move. The website and repair email show these actions separately from the original run outcome.
