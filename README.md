...
- **Inbounds to link** lists receiving appointments created for missing 311, 660 or ITRN/ITRA references. Only a structured 404 lookup permits this fallback; authentication and server errors stop for review. The appointment uses the saved time, door group, carrier and RCV type, with the original reference and “Not in system when appointment was made” in its notes. A deterministic external appointment reference checks for a matching earlier bot appointment before creation. Changed, active or ambiguous matches stop. The saved appointment details are read back before reporting success. The nonexistent truck is not assigned; an operator links it manually when available. The website, CSV export and email retain this follow-up list. Preflight checks and writes are separate API requests; do not launch overlapping production runs or retry an uncertain submission.
