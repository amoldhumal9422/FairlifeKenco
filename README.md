...
- Ordinary appointment conflicts stop before wave creation and use read-only lookups. **Repair appointment** is a separate, confirmed production action for one outbound AZ02 load. It checks allocation, ownership, the saved shipment and empty picks first. It then reuses an already correct appointment or moves the existing appointment to the saved date, time and door group, retaining its ID. Both the appointment and load assignment must be verified before creating a missing wave. Wave creation uses the saved name and shipment, rechecks that the name is unused and the load has no wave, and waits for completion before verifying every link. A correctly linked unallocated wave is reused. A different wave name stops for review; this repair does not delete waves, deassign loads or delete/recreate appointments. The workbook is not rerun.
...
- Run results retain their original production totals. Repair history records the saved wave and shipment, original appointment time, verified appointment time and door group, appointment reuse or movement, wave creation or reuse, and verified links. A failed appointment update stops before wave creation. If wave setup fails afterward, the verified appointment change remains recorded as partial completion and the run requires manual review. The website and repair email show these actions separately from the original run outcome. Historical deletion fields remain visible for older repairs.
